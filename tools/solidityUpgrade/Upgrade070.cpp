	...
	if (auto const funcType = dynamic_cast<FunctionType const*>(type))
	{
		if (funcType->valueSet())
			m_changes.emplace_back(
				...
				SourceTransform{m_scanner}.valueUpdate(_functionCall.location())
			);
		...
		if (funcType->gasSet())
			m_changes.emplace_back(
				UpgradeChange::Level::Safe,
				_functionCall.location(),
				SourceTransform{m_scanner}.gasUpdate(_functionCall.location())
			);
	}
}
	...
	if (
		...
		if (magicVar->type()->category() == Type::Category::Integer)
		{
			...
			m_changes.emplace_back(
				...
				SourceTransform{m_scanner}.nowUpdate(_identifier.location())
			);
		}
}
	...
	for (FunctionDefinition const* function: _contract.definedFunctions())
		if (function->isConstructor() && !function->noVisibilitySpecified())
			m_changes.emplace_back(
				UpgradeChange::Level::Safe,
				function->location(),
				SourceTransform{m_scanner}.removeVisibility(function->location())
			);
}
