...
void UpgradeChange::log(ScannerBySourceName const& _scanner, bool const _shorten) const
{
	stringstream os;
	SourceReferenceFormatter formatter{os, _scanner, true, false};
	...
	os << "=======================" << endl;
	formatter.printSourceLocation(SourceReferenceExtractor::extract(_scanner, &m_location));
	os << endl;

	LineColumn lineEnd = _scanner.scanner(*m_location.sourceName).translatePositionToLineColumn(m_location.end);
