...
{
public:
	explicit SourceTextRetriever(langutil::ScannerBySourceName const& _scanner): m_scanner(_scanner) {}
	std::string text(langutil::SourceLocation const& _location) const
	{
		solAssert(_location.hasText(), "");
		return m_scanner.scanner(*_location.sourceName).source().substr(
			static_cast<size_t>(_location.start), static_cast<size_t>(_location.end - _location.start)
		);
	}
protected:
...
public:
	...
	{
		bool isMultiline = SourceAnalysis{m_scanner}.isMultilineKeyword(_location, _keyword);
