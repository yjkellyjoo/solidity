...
void AbstractContract::endVisit(ContractDefinition const& _contract)
{
	...
	if (
		...
		m_changes.emplace_back(
				...
				_contract.location(),
				SourceTransform{m_scanner}.insertBeforeKeyword(_contract.location(), "contract", "abstract")
		);
}
...
)
{
	...
	if (SourceAnalysis{m_scanner}.hasVirtualKeyword(location))
		upgradedCode = SourceTransform{m_scanner}.insertAfterKeyword(
			location,
			"virtual",
			overrideExpression
		);
	else if (SourceAnalysis{m_scanner}.hasMutabilityKeyword(location))
		upgradedCode = SourceTransform{m_scanner}.insertAfterKeyword(
			location,
			stateMutabilityToString(_function.stateMutability()),
			overrideExpression
		);
	else if (SourceAnalysis{m_scanner}.hasVisibilityKeyword(location))
		upgradedCode = SourceTransform{m_scanner}.insertAfterKeyword(
			location,
		...
		upgradedCode = SourceTransform{m_scanner}.insertAfterRightParenthesis(
			location,
			overrideExpression
		);
...
string VirtualFunction::appendVirtual(FunctionDefinition const& _function) const
{
	...
	if (SourceAnalysis{m_scanner}.hasMutabilityKeyword(location))
		upgradedCode = SourceTransform{m_scanner}.insertAfterKeyword(
			location,
			stateMutabilityToString(_function.stateMutability()),
			"virtual"
